...
class CompositeVolMetric(Metric):
    ...
    def __init__(self, base_loss: Metric, high_q: float = 0.85,
                 penalty_weight: float = 0.15, warmup_epochs: int = 2,
    ...
    def forward(self, y_pred, target, **kwargs):
        ...
        try:
            ...
            # focus on highest tail (slightly wider than before)
            ...
            if torch.any(high_mask):
                # relative underestimation (only when pred < target)
                under = torch.clamp(target_dec - pred_med_dec, min=0.0)
                rel_under = under / (target_dec + 1e-12)
                # intensity weighting grows with size of the spike
                intensity = (target_dec / (thresh + 1e-12)).pow(0.5)
                peak_loss = ((rel_under ** 2) * intensity)[high_mask].mean()
        ...
        try:
            cal_thresh = torch.quantile(torch.sinh(target_vol).detach(), 0.60)
            ...
            if torch.any(adj_mask) and float(self.cal_weight) > 0:
                # bring predictions halfway to target, weighted by spike intensity
                gap = (target_dec2[adj_mask] - pred_med_dec2[adj_mask])
                intensity = (target_dec2[adj_mask] / (cal_thresh + 1e-12)).pow(0.5)
                adj_pred = pred_med_dec2[adj_mask] + 0.5 * gap
                cal_loss = ( (adj_pred - target_dec2[adj_mask]) ** 2 * intensity ).mean()
        ...
        try:
            ...
            if g_cpu is not None and yv_cpu is not None and pv_cpu is not None:
                ...
                df_out = pd.DataFrame({
                    ...
                    "y_vol_pred_raw": pv_dec.numpy().tolist(),
                })
                ...
                try:
                    ...
                    df_out["y_vol_pred"] = pv_cal_np.tolist()
                except Exception:
                    pass
                if yd_cpu is not None and pd_cpu is not None and yd_cpu.numel() > 0 and pd_cpu.numel() > 0:
