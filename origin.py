    ...
    def on_fit_end(self, trainer, pl_module):
        ...
        try:
            ...
            if g_cpu is not None and yv_cpu is not None and pv_cpu is not None:
                ...
                # NOTE: No calibration is applied to validation parquet — save raw decoded q50 predictions
                ...
                df_out = pd.DataFrame({
                    ...
                    "y_vol_pred": pv_dec.numpy().tolist(),          # point forecast ~ q50 (raw, not calibrated)
                })
...
# --- Rolling-origin / walk-forward flags ---
parser.add_argument("--rolling_eval", type=lambda s: str(s).lower() in ("1","true","t","yes","y","on"),
                    default=False, help="Enable rolling-origin evaluation loop (walk-forward)")
parser.add_argument("--roll_start", type=str, default=None,
                    help="Absolute start date for first training window (e.g. '2020-04-01'); default = earliest")
parser.add_argument("--roll_step_days", type=int, default=90,
                    help="How far to move the origin forward each fold")
parser.add_argument("--roll_val_days", type=int, default=90,
                    help="Validation window length for each fold")
parser.add_argument("--roll_min_train_days", type=int, default=365,
                    help="Minimum expanding training window before first fold")
parser.add_argument("--roll_max_folds", type=int, default=8,
                    help="Hard cap on number of folds")
parser.add_argument("--roll_pred_len", type=int, default=1,
                    help="TFT max_prediction_length for rolling folds (1 for 1-step ahead)")
parser.add_argument("--roll_keep_checkpoints", type=lambda s: str(s).lower() in ("1","true","t","yes","y","on"),
                    default=False, help="Keep all fold checkpoints (otherwise keep last+best only)")

# --- Rolling export & in-sample capture knobs ---
parser.add_argument("--export_test_after_rolling", type=lambda s: str(s).lower() in ("1","true","t","yes","y","on"),
                    default=True, help="After rolling folds, refit on all pre-test data and export predictions on the fixed 10% TEST")
parser.add_argument("--save_insample_after_fold", type=lambda s: str(s).lower() in ("1","true","t","yes","y","on"),
                    default=False, help="At the end of each fold, export TRAIN (in-sample) predictions & metrics for that fold")
...
def _universal_available():
    """Return a path string to universal_data.parquet if available (GCS preferred), else None."""
    try:
        if fs is not None and fs.exists(str(UNIVERSAL_URI)):
            return str(UNIVERSAL_URI)
    except Exception:
        pass
    try:
        if LOCAL_UNIVERSAL is not None and Path(LOCAL_UNIVERSAL).exists():
            return str(LOCAL_UNIVERSAL)
    except Exception:
        pass
    return None
...
# Optional: single complete parquet holding the whole history
UNIVERSAL_URI = f"{GCS_DATA_PREFIX}/universal_data.parquet"

# Local fallback for the universal parquet
...
LOCAL_UNIVERSAL = _local_cleaned_dir() / "universal_data.parquet"
...
# Choose the source of truth for READ_PATHS (three-way split), with universal fallback
if _all_exists_gcs([TRAIN_URI, VAL_URI, TEST_URI]):
    ...
    uni_p = _universal_available()
    if uni_p is not None:
        # load_all_splits_for_training() will interpret this appropriately
        READ_PATHS = [uni_p, uni_p, uni_p]
    else:
        raise FileNotFoundError(
            "Could not locate the dataset. Checked GCS URIs and the local default. "
            "Provide --data_dir with universal_train/val/test.parquet or universal_data.parquet, "
...
def generate_rolling_folds(df: pd.DataFrame,
    ...
    while True:
        val_start = origin
        val_end   = val_start + pd.Timedelta(days=int(val_days))
        if hard_val_end is not None:
            val_end = min(val_end, hard_val_end)
        if val_start >= t_max or val_end <= val_start:
            break
        ...
        yield {
            ...
            "t_train_end": val_start,
            "t_val_start": val_start,
            "t_val_end": val_end,
        }
        folds += 1
        if folds >= int(max_folds):
            break
...
def _train_single_fold(train_df: pd.DataFrame, val_df: pd.DataFrame, fold_id: int):
    training, validation = _build_datasets_for_split(
        train_df, val_df,
        max_encoder_length=MAX_ENCODER_LENGTH,
        max_prediction_length=int(getattr(ARGS, "roll_pred_len", MAX_PRED_LENGTH)),
    )

    num_workers = int(getattr(ARGS, "num_workers", os.cpu_count() - 1 or 0))
    ...
    try:
        ...
        if "categorical_encoders" in enc and GROUP_ID[0] in enc["categorical_encoders"]:
            ce = enc["categorical_encoders"][GROUP_ID[0]]
            if hasattr(ce, "classes_"):
                id_to_name = {int(i): str(v) for i, v in enumerate(list(ce.classes_))}
    except Exception:
        pass

    per_asset_cb = PerAssetMetrics(id_to_name=id_to_name or {0: "asset"},
                                   vol_normalizer=training.target_normalizer.normalizers[0],
                                   max_print=10)

    fold_dir = LOCAL_RUN_DIR / f"fold_{fold_id}"
    fold_ckpt = fold_dir / "checkpoints"
    fold_logs = fold_dir / "lightning_logs"
    ...
    try:
        last_path = fold_ckpt / "last.ckpt"
        trainer.save_checkpoint(str(last_path))
        if not getattr(ARGS, "roll_keep_checkpoints", False):
            best_name = os.path.basename(getattr(trainer.checkpoint_callback, "best_model_path", "") or "")
            for p in fold_ckpt.glob("*.ckpt"):
                if p.name not in {"last.ckpt", best_name}:
    ...
    return {
        "fold_id": int(fold_id),
        "val_mae": float(trainer.callback_metrics.get("val_mae_overall", float("nan"))),
        "val_rmse": float(trainer.callback_metrics.get("val_rmse_overall", float("nan"))),
        "val_qlike": float(trainer.callback_metrics.get("val_qlike_overall", float("nan"))),
...
def run_rolling_origin(train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame, uni_df: pd.DataFrame | None):
    ...
    for asset, g in base.groupby(GROUP_ID[0], observed=True, sort=False):
        ...
        pre_list.append(g[g[TIME_COL] < cut])
        test_list.append(g[g[TIME_COL] >= cut])
    ...
    for fold in generate_rolling_folds(
        pre_df,
        time_col=TIME_COL,
        start_date=getattr(ARGS, "roll_start", None),
        min_train_days=int(getattr(ARGS, "roll_min_train_days", 365)),
        val_days=int(getattr(ARGS, "roll_val_days", 90)),
        step_days=int(getattr(ARGS, "roll_step_days", 90)),
        max_folds=int(getattr(ARGS, "roll_max_folds", 8)),
        hard_val_end=hard_end,
    ):
        f = int(fold["fold_id"])
        print(f"\n===== ROLLING FOLD {f} =====")
        print(f"Train ≤ {fold['t_train_end'].date()} | Val {fold['t_val_start'].date()} → {fold['t_val_end'].date()}")
        tr_df = pre_df.loc[fold["train_mask"]].copy()
        va_df = pre_df.loc[fold["val_mask"]].copy()
        ...
        if getattr(ARGS, "save_insample_after_fold", False):
            try:
                ...
                ins_path = LOCAL_OUTPUT_DIR / f"fold_{f}_train_predictions.parquet"
                _export_split_from_best(trainer, train_loader_ins, split="train", out_path=ins_path, calibrator=None)
            except Exception as e:
    ...
    if metrics:
        ...
        agg_path = LOCAL_OUTPUT_DIR / "rolling_eval_summary.json"
        csv_path = LOCAL_OUTPUT_DIR / "rolling_eval_folds.csv"
        with open(agg_path, "w") as f:
            json.dump({"folds": metrics, "mean": agg}, f, indent=2)
        met_df.to_csv(csv_path, index=True)
        print(f"✓ Saved rolling summary → {agg_path}")
        print(f"✓ Saved per-fold metrics → {csv_path}")
        try:
            upload_file_to_gcs(str(agg_path), f"{GCS_OUTPUT_PREFIX}/rolling_eval_summary.json")
...
if __name__ == "__main__":
        ...
        # Load data for either rolling or single-run
    train_df, val_df, test_df, uni_df = load_all_splits_for_training()

    # If requested, run rolling-origin evaluation using train+val (or universal) while keeping last 10% as test
    if getattr(ARGS, "rolling_eval", False):
        print(">>> Rolling-origin evaluation ENABLED (keeping final 10% as fixed test)")
        run_rolling_origin(train_df, val_df, test_df, uni_df)
        raise SystemExit(0)
    ...
    def load_all_splits_for_training():
        """
        Returns (train_df, val_df, test_df, universal_df_or_none).
        When --rolling_eval is on, we keep the last 10% chronologically as a fixed test set
        and perform rolling folds inside the pre-test region using train+val (or universal minus test).
        """
        # If READ_PATHS all point to one file (universal), load once; else load split files
        if len(set(map(str, READ_PATHS))) == 1:
            uni_df = load_split(READ_PATHS[0])
            tdf, vdf, sdf = _chronological_resplit(uni_df, train_frac=0.8, val_frac=0.1)
            return tdf, vdf, sdf, uni_df
        else:
            train_df = load_split(READ_PATHS[0])
            val_df   = load_split(READ_PATHS[1])
            test_df  = load_split(READ_PATHS[2])
            uni_path = _universal_available()
            uni_df = load_split(uni_path) if uni_path is not None else None
            return train_df, val_df, test_df, uni_df
